...
<f:view>
  ...
  <tiles:definition id="myReportEditBrowseUserComponentsDefinition" extends="inquiryCenterBrowseDataPickerDefinition">
    <tiles:put name="applicationTitle" value="${bundle.inquiry_center_title}" />
    <tiles:put name="pageTitle">
      <h:outputFormat value="#{bundle.my_report_edit_browse_user_components_title}" >
        <f:param id="RptFmt" value="#{myReportsBean.selectedReport.title}" />
      </h:outputFormat>
    </tiles:put>
    <tiles:put name="selectedItemBoxLabel">
      <h:outputText id="slctItmBxLblTxt" value="#{bundle.my_report_edit_browse_user_components_selected_items_box_header}" />:
    </tiles:put>
  ...
  <c:set var="boldItalicizedStyleClassId" value="selectableItemGroupAlternate" scope="request" />
  
  <%-- Insert Browse User Components Definition --%>
  <f:subview id="myReportEditBrowseUserComponentsDefinitionSubview">
    	<tiles:insert beanName="myReportEditBrowseUserComponentsDefinition" flush="false" />
  </f:subview>    
</f:view>