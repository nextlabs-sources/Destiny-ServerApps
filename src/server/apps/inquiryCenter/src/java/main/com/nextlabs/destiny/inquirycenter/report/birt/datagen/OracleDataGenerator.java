...
/**
 * <p>
 * OracleDataGenerator for ORACLE specific SQL generation
 * </p>
 * 
 * 
 * @author Amila Silva
 * 
 */
public final class OracleDataGenerator extends AbstractDataGenerator {
	...
	public OracleDataGenerator(Session session) {
		super(session);
	}
    ...
    // || mappingDO.getColumnName().length() < 1) {
    //
    // sBuf.append(replaceInvalidChar(colName))
    // .append(", ");
    //
    // // max(case when ATTRIBUTE_NAME = 'grouping' then ATTRIBUTE_VALUE
    // // end) as attr21,
    //
    // sBufTemp.append(" max(case when ").append(attrbIdString)
    // .append("=").append(mappingDO.getId()).append(" then ")
    // .append(attrbIdString).append(" end) as ")
    // .append(replaceInvalidChar(colName))
    // .append(", ");
    //
    // isEAV = true;
    // } else {
