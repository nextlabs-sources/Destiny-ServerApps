...
import com.nextlabs.authentication.config.listeners.ConfigUpdateListener;
import com.nextlabs.authentication.config.listeners.LoggerUpdateListener;
import com.nextlabs.authentication.config.properties.ActiveMQConnectionFactoryProperties;
...
public class ConfigRefresher implements Runnable {
    ...
    @Value("${jms.config.update.topic:jms/cc.cas.config.update}")
    private String configUpdateTopic;

    @Value("${jms.logger.update.topic:jms/cc.cas.logger.update}")
    ...
    public void run() {
        LOGGER.info("Starting refresher: Configurations={}, Loggers={}",
                configUpdateRefresherEnabled, loggerUpdateRefresherEnabled);
